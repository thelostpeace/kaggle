...
X_train, X_val, y_train, y_val = train_test_split(train_X, y, train_size=0.8)
split_index = [-1 if x in X_train.index else 0 for x in train_X.index]
...
model = GridSearchCV(DecisionTreeClassifier(), param_grid=params, cv=pds, verbose=16)
model.fit(train_X, y)
